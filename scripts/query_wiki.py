...
# Context constraints - Provider aware
if LLM_PROVIDER == "gemini":
    MAX_CONTEXT_TOKENS = int(os.environ.get("MAX_CONTEXT_TOKENS", "100000"))
    RESERVED_OUTPUT_TOKENS = int(os.environ.get("RESERVED_OUTPUT_TOKENS", "4096"))
else:
...
PROMPT_SAFETY_MARGIN = int(os.environ.get("PROMPT_SAFETY_MARGIN", "500"))
...
def get_gemini_response(messages: List[Dict[str, str]]) -> str | None:
    """Call Google Gemini API via REST with proper system_instruction support."""
    ...
    payload = {
        ...
        "generationConfig": {
            "temperature": 0.1,
            "maxOutputTokens": 4096,  # Increased for deep analysis
        },
        "safetySettings": [
    ...
    try:
        ...
        if "candidates" not in data or not data["candidates"]:
            logger.error(f"Gemini API returned no candidates. Data: {json.dumps(data)}")
...
def get_llm_response(
    messages: List[Dict[str, str]], provider: str = None
) -> str | None:
    ...
    # Default to OpenAI-compatible (MLX, DeepSeek, etc.)
    api_key = (
    ...
    try:
        ...
        logger.error(f"LLM Error ({current_provider}): {e}")
        return None
